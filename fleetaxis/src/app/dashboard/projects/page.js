...
export default function ProjectsPage() {
  ...
  return (
    <DashboardLayout>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Sora:wght@400;500;600;700&family=JetBrains+Mono:wght@500&display=swap');*{box-sizing:border-box}input{font-family:'Sora',sans-serif}input:focus{outline:none}::-webkit-scrollbar{width:4px}::-webkit-scrollbar-thumb{background:#E2E8F0;border-radius:2px}@keyframes fadeUp{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}`}</style>
      <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(340px, 1fr))", gap: 14 }}>
            {projects.map(p => {
              const pt = PROJECT_TYPES.find(t => t.id === p.type) || PROJECT_TYPES[0];
              return (
                <div key={p.id} onClick={() => router.push("/dashboard")} style={{ background: "#fff", border: "1px solid #F0F4F8", borderRadius: 14, padding: "16px", cursor: "pointer", transition: "all .2s", boxShadow: "0 2px 8px rgba(0,0,0,.04)" }}
                  onMouseEnter={e => { e.currentTarget.style.borderColor = pt.color; e.currentTarget.style.transform = "translateY(-2px)"; }}
                  onMouseLeave={e => { e.currentTarget.style.borderColor = "#F0F4F8"; e.currentTarget.style.transform = "none"; }}>
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 12 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: "#0F172A", lineHeight: 1.4, marginBottom: 4 }}>{p.name}</div>
                      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                        <div style={{ width: 6, height: 6, borderRadius: "50%", background: STATUS_COLORS[p.status] }} />
                        <span style={{ fontSize: 11, color: STATUS_COLORS[p.status], fontWeight: 500, textTransform: "capitalize" }}>{p.status}</span>
                        <span style={{ fontSize: 11, color: "#94A3B8" }}>· {p.date}</span>
                      </div>
                    </div>
                  </div>
                  <div style={{ marginBottom: 12 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}><span style={{ fontSize: 11, color: "#64748B" }}>Progress</span><span style={{ fontSize: 11, fontWeight: 600, color: pt.color }}>{p.progress}%</span></div>
                    <div style={{ background: "#F0F4F8", borderRadius: 99, height: 5 }}><div style={{ width: `${p.progress}%`, height: "100%", background: pt.color, borderRadius: 99 }} /></div>
                  </div>
            ...
            <div onClick={() => setShowNew(true)} style={{ background: "#F8FAFB", border: "2px dashed #E2E8F0", borderRadius: 14, padding: "16px", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: 180 }}
      ...
      {showNew && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}>
          <div style={{ background: "#fff", borderRadius: 20, padding: "28px", width: 440, animation: "fadeUp .3s ease" }}>
            ...
            <input value={newProject.name} onChange={e => setNewProject({ ...newProject, name: e.target.value })} placeholder="e.g. EV Transition Study — Client Name"
              style={{ width: "100%", padding: "11px 13px", borderRadius: 10, border: "1.5px solid #E2E8F0", fontSize: 13, marginBottom: 14, fontFamily: "'Sora',sans-serif" }}
              onFocus={e => e.target.style.borderColor = "#00C16E"} onBlur={e => e.target.style.borderColor = "#E2E8F0"} />
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 20 }}>
              {PROJECT_TYPES.map(t => (
                <div key={t.id} onClick={() => setNewProject({ ...newProject, type: t.id })} style={{ padding: "8px 10px", borderRadius: 9, border: `2px solid ${newProject.type === t.id ? t.color : "#E2E8F0"}`, background: newProject.type === t.id ? `${t.color}10` : "#fff", cursor: "pointer", textAlign: "center" }}>
                  <div style={{ fontSize: 18, marginBottom: 3 }}>{t.icon}</div>
                  <div style={{ fontSize: 10, fontWeight: newProject.type === t.id ? 600 : 400, color: newProject.type === t.id ? t.color : "#64748B" }}>{t.label}</div>
                </div>
              ))}
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setShowNew(false)} style={{ flex: 1, padding: "11px", borderRadius: 10, border: "1.5px solid #E2E8F0", background: "#fff", fontSize: 13, cursor: "pointer", fontFamily: "'Sora',sans-serif" }}>Cancel</button>
              <button onClick={() => { if (newProject.name) { setProjects(prev => [...prev, { id: Date.now(), name: newProject.name, type: newProject.type, status: "draft", files: 0, messages: 0, date: new Date().toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" }), progress: 0 }]); setShowNew(false); setNewProject({ name: "", type: "ev" }); } }} disabled={!newProject.name}
