...
export default function DashboardLayout({ children }) {
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: "#F0F4F8", fontFamily: "'Sora',sans-serif", overflow: "hidden" }}>
      <div style={{ width: collapsed ? 64 : 220, flexShrink: 0, background: "#0B1437", display: "flex", flexDirection: "column", transition: "width .25s ease", position: "relative", zIndex: 10 }}>
        <div style={{ padding: collapsed ? "20px 14px" : "20px 20px", borderBottom: "1px solid rgba(255,255,255,.06)", display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }} onClick={() => router.push("/dashboard")}>
          <div style={{ width: 34, height: 34, background: "linear-gradient(135deg,#00C16E,#009955)", borderRadius: 9, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "#fff", flexShrink: 0 }}>FA</div>
          {!collapsed && <div><div style={{ fontSize: 14, fontWeight: 700, color: "#fff" }}>FleetAxis<span style={{ color: "#00C16E" }}> AI</span></div><div style={{ fontSize: 10, color: "rgba(255,255,255,.35)" }}>Fleet Intelligence OS</div></div>}
        </div>
        {!collapsed && <div style={{ margin: "12px 12px 0", background: "rgba(255,255,255,.05)", borderRadius: 10, padding: "10px 12px", border: "1px solid rgba(255,255,255,.06)" }}><div style={{ fontSize: 10, color: "rgba(255,255,255,.35)", marginBottom: 3 }}>ORGANISATION</div><div style={{ fontSize: 12, fontWeight: 600, color: "#fff" }}>GreenLine Logistics</div></div>}
          ...
          {NAV_ITEMS.map(item => {
            ...
            return (
              <div key={item.id} onClick={() => !item.soon && router.push(item.path)}
                ...
                {!collapsed && <><span style={{ fontSize: 13, fontWeight: active ? 600 : 400, color: active ? "#00C16E" : item.soon ? "rgba(255,255,255,.3)" : "rgba(255,255,255,.7)", flex: 1 }}>{item.label}</span>{item.soon && <span style={{ fontSize: 9, background: "rgba(255,255,255,.08)", color: "rgba(255,255,255,.3)", padding: "1px 6px", borderRadius: 99 }}>SOON</span>}</>}
              </div>
            );
          })}
        </div>
          ...
          {BOTTOM_ITEMS.map(item => {
            ...
            return (
              <div key={item.id} onClick={() => router.push(item.path)}
                ...
                {!collapsed && <span style={{ fontSize: 13, color: active ? "#00C16E" : "rgba(255,255,255,.5)", fontWeight: active ? 600 : 400 }}>{item.label}</span>}
              </div>
            );
          })}
          <div style={{ margin: "8px", padding: "10px 12px", background: "rgba(255,255,255,.04)", borderRadius: 10, display: "flex", alignItems: "center", gap: 10 }}>
            ...
            {!collapsed && <div><div style={{ fontSize: 12, fontWeight: 600, color: "#fff" }}>Waheed Syed</div><div style={{ fontSize: 10, color: "rgba(255,255,255,.4)" }}>Admin</div></div>}
          </div>
        </div>
